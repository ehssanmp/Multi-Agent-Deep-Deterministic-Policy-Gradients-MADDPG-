...
class MADDPG:
    ...
    def learn(self, memory: MultiAgentReplayBuffer):
        ...
        actor_states, states, actions, rewards, \
            actor_new_sates, states_, dones = memory.sample_buffer()
        ...
        for agent_idx, agent in enumerate(self.agents):
            new_states = T.tensor(actor_new_sates[agent_idx],
                                  dtype=T.float).to(device)
            new_pi = agent.target_actor.forward(new_states)
            all_agents_new_actions.append(new_pi)
            
            current_states = T.tensor(actor_states[agent_idx],
                                      dtype=T.float).to(device)
            pi = agent.actor.forward(current_states)
